app.controller("indicesCtrl", function ($scope, $http, $rootScope) {
    $scope.indexData = [];
    $rootScope.toolBarName = "Indices";
    host = $rootScope.hostSelected;
    $http.get("http://"+host+":9200/_cat/indices")
            .then(function(response) {
            var indexResponse = response.data;
            console.log('########## Response Data shards Response Response ##########');
            console.log(indexResponse);
            console.log('########## Converted Response Data Disk Allocation Response ##########');
            $scope.indexData = covertToJSON(indexResponse);
    ...
    function covertToJSON(rowData){
        ...
        var keys = ["Status","State","Index","Id"];
